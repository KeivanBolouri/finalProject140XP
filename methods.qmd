...
**H₀:** Reporting uncertainty does not differ between Somalia and Yemen.\
**H₁:** Reporting uncertainty differs between Somalia and Yemen.
...
\begin{aligned}
H_0 &: \text{dispersion} = 1 \quad \text{(Poisson model is adequate)}, \\
H_1 &: \text{dispersion} > 1 \quad \text{(overdispersion present)}.
\end{aligned}
...
ggplot() +
  ...
  labs(
    title = "Predicted Civilian Casualties (H1 Model)",
...
### Interpretation of H1 Model Results

This discrepancy suggests that additional unmeasured factors—such as target type, population density, intelligence quality, or operational context—likely influence civilian harm in Yemen. Therefore, the model’s overall comparative conclusions remain valid, but predictions for Yemen should be interpreted cautiously due to the region’s greater variability.
...
ggplot(newdata_h2, aes(x = drone_label, y = pred_civ, fill = region)) +
  ...
  labs(
    title    = "H2: Drone Effect on Civilian Casualties by Region",
    subtitle = "Bars = model-predicted means; points = observed means",
    x        = "Strike type",
...
ggplot(newdata_h2, aes(x = drone_label, y = pred_civ, color = region, group = region)) +
  ...
  geom_point(size = 4) +
  labs(
    title = "H2 Interaction: Drone Use × Region",
