...
\textbf{H₀:} There is no difference in civilian casualty rates between
Somalia and Yemen.\\
\textbf{H₁:} Civilian casualty rates differ between Somalia and Yemen.
...
\NormalTok{vars }\OtherTok{\textless{}{-}} \FunctionTok{data.frame}\NormalTok{(}
  \AttributeTok{Variable =} \FunctionTok{c}\NormalTok{(}\StringTok{"Civilian casualties"}\NormalTok{, }\StringTok{"Region"}\NormalTok{, }\StringTok{"Drone"}\NormalTok{, }\StringTok{"US confirmed"}\NormalTok{,}
               \StringTok{"Minimum strikes"}\NormalTok{, }\StringTok{"Total killed"}\NormalTok{),}
  \AttributeTok{Description =} \FunctionTok{c}\NormalTok{(}
    \StringTok{"Number of civilians reported killed in the strike (outcome variable)."}\NormalTok{,}
    \StringTok{"Country where the strike occurred (Somalia or Yemen)."}\NormalTok{,}
    \StringTok{"Indicates whether the strike was carried out by a drone (1 = drone)."}\NormalTok{,}
    \StringTok{"Whether the strike was officially confirmed by the U.S. government."}\NormalTok{,}
    \StringTok{"Minimum number of strike events associated with the record."}\NormalTok{,}
    \StringTok{"Minimum number of total fatalities (civilians + militants)."}
\NormalTok{  )}
\NormalTok{)}

\FunctionTok{kable}\NormalTok{(vars, }\AttributeTok{caption =} \StringTok{"Variable Definitions"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\textbf{H₀:} Drone strikes do not affect civilian casualties differently
in Somalia and Yemen.\\
\textbf{H₁:} The effect of drone strikes on civilian casualties differs
...
{[} \text{civilian\_casualties} \sim \text{drone} * \text{region} +
...
To assess whether casualty reporting uncertainty differs across regions,
we model uncertainty (defined as the difference between maximum and
minimum reported deaths):

{[} \text{uncertainty\_killed} \sim \text{region} + \text{drone} +
\text{us\_confirmed} + \text{min\_strikes} {]}
